...
START = f'Добро пожаловать в бот "temp_diz". ' \
        f'\nЯ создан для того, чтобы помогать тебе пиарить музон в ТикТоке. ' \
        f'\n\nПеред началом использования бота, обязательно прочитай небольшую инструкцию по использованию бота, ' \
        f'а также правила - /help'
...
def MONEYS(payment_money_amount, payment_comment):
    ...
    message = f'Денег будет добавлено на аккаунт: {payment_money_amount}\n' \
              ...
              f'Коммент, который нужно указать в транзакции: {payment_comment} '
    return message
