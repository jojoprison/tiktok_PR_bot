...
def save_channel(**writer):
    if writer['writer'] and 'channel_id' in writer:
        ...
        conn.execute(
            '''INSERT INTO channels(number, id, writer, status, checked_channels, fined) VALUES(?,?,?,?,?,?)''',
            (number, writer['channel_id'], writer['writer'], 0, str([]), str([]),))
        conn.commit()
        return number
    if writer['subs_count'] and writer['writer']:
        ...
        conn.execute('''UPDATE channels SET subscriptions = ?, subs_count = ? WHERE number = ?''',
                     (str({}), writer['subs_count'], number,))
...
def get_channel_stat(writer):
    ...
    number = number.fetchall()[0][0]
    stat_list = conn.execute('''SELECT subs_count, id FROM channels WHERE writer = ? AND number = ?''',
                             (writer, number,))
...
def edit_promotion_status(number, status):
    sql = conn.execute('''SELECT COUNT(number), writer, subscriptions, subs_count FROM channels WHERE number = ?''',
                       (number,))
